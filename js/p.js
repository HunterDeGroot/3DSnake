...
function initSnake() {
	...
	for( var i = SNAKESTARTSIZE - 2; i >= 0; i--) {
		...
		// Alternate scales
		if(i % 2 == 0)	material = new THREE.MeshLambertMaterial( { map: scales1 } );
		else			material = new THREE.MeshLambertMaterial( { map: scales2 } );
...
function addHead() {
    ...
    //texture.wrapS = texture.wrapT = THREE.RepeatWrapping;
    material = new THREE.MeshLambertMaterial( { map: hScales } );
	var head = new THREE.Mesh(hGeometry, material );
	geometry = new THREE.SphereGeometry(UNITSIZE/4);
	material = new THREE.MeshLambertMaterial( { map: eyeT } );
...
function grow() {
    ...
    //texture.wrapS = texture.wrapT = THREE.RepeatWrapping;
	if((snake.length + score) % 2 == 0)	material = new THREE.MeshLambertMaterial( { map: scales1 } );
	else			material = new THREE.MeshLambertMaterial( { map: scales2 } );	
